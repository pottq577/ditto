...
export const CameraPage = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const sendSticker = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/v1/stickers?userId=1&coupleId=1', {
        method: 'POST',
        body: formData,
      });
  ...
  if (photoUri) {
    return (
      <View style={styles.container}>
        ...
        {isProcessing && <ActivityIndicator style={styles.loader} size="large" color="#ffffff" />}
        <View style={styles.actionRow}>
          <TouchableOpacity style={styles.button} onPress={retake} disabled={isProcessing}>
            <Text style={styles.buttonText}>다시 찍기</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.button, styles.sendButton]} onPress={sendSticker} disabled={isProcessing}>
